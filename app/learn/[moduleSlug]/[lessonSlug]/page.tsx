...
function InstructionContent({ step }: { step: LessonStep }) {
  ...
  return (
    <div className="space-y-6">
      {/* Badge for step type */}
      {step.type === 'challenge' && (
        <div className="inline-flex items-center gap-2 px-3 py-1 bg-atrisk-bg text-atrisk-fg rounded-full text-xs font-bold uppercase tracking-wide">
          <span className="w-2 h-2 rounded-full bg-atrisk"></span>
          Challenge
        </div>
      )}
      {step.type === 'recap' && (
        <div className="inline-flex items-center gap-2 px-3 py-1 bg-verified-bg text-verified-fg rounded-full text-xs font-bold uppercase tracking-wide">
      ...
      {/* Heading */}
      <h2 className="text-2xl font-bold text-maru-navy">{instruction.heading}</h2>

      {/* Video Embed */}
      {instruction.videoUrl && (
        <div className="rounded-xl overflow-hidden shadow-lg border border-maru-line aspect-video w-full bg-black">
          <iframe
      ...
      <div className="prose prose-gray max-w-none">
        {instruction.body.split('\n\n').map((paragraph, i) => (
          <p key={i} className="text-maru-grey leading-relaxed whitespace-pre-line">
            {paragraph.split(/(\*\*[^*]+\*\*)/).map((part, j) => {
              if (part.startsWith('**') && part.endsWith('**')) {
                return <strong key={j} className="text-maru-navy">{part.slice(2, -2)}</strong>;
              }
              return part;
            })}
          </p>
      ...
      {instruction.tips && instruction.tips.length > 0 && (
        <div className="bg-maru-blue-100 border border-maru-blue-100 rounded-xl p-5">
          <h3 className="font-semibold text-maru-navy mb-3 flex items-center gap-2">
            <Lightbulb className="w-4 h-4 text-maru-blue-700" />
            Pro Tips
          </h3>
          <ul className="space-y-2">
            {instruction.tips.map((tip, i) => (
              <li key={i} className="flex items-start gap-2 text-sm text-maru-blue-700">
                <span className="text-maru-blue-300 mt-0.5">•</span>
                {tip}
              </li>
            ))}
          </ul>
      ...
      {instruction.examples && (
        <div className="space-y-3">
          {instruction.examples.bad && (
            <div className="border-l-4 border-overdue pl-4 py-2 bg-overdue-bg rounded-r-lg">
              <p className="text-xs text-overdue-fg font-semibold mb-1 flex items-center gap-1">
                <AlertTriangle className="w-3 h-3" />
                Bad Example
              </p>
              <p className="text-maru-grey text-sm italic">"{instruction.examples.bad}"</p>
            </div>
          )}
          {instruction.examples.good && (
            <div className="border-l-4 border-verified pl-4 py-2 bg-verified-bg rounded-r-lg">
              <p className="text-xs text-verified-fg font-semibold mb-1 flex items-center gap-1">
                <CheckCircle className="w-3 h-3" />
                Good Example
              </p>
              <p className="text-maru-grey text-sm italic">"{instruction.examples.good}"</p>
            </div>
          )}
        </div>
      )}
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center bg-maru-cloud text-center p-8">
      <div className={cn(
        ...
        isRecap ? "bg-verified-bg" : "bg-maru-blue-100"
      )}>
        <div className="absolute inset-0 rounded-full animate-pulse opacity-30" 
             style={{ background: isRecap ? 'radial-gradient(circle, var(--maru-verified) 0%, transparent 70%)' : 'radial-gradient(circle, var(--maru-blue) 0%, transparent 70%)' }}>
        </div>
        {isRecap ? (
          <CheckCircle className="w-20 h-20 text-verified-fg" />
        ) : (
          <BookOpen className="w-20 h-20 text-maru-blue-700" />
        )}
      </div>
      
      <h3 className="text-xl font-bold text-maru-navy mb-2">
        {step.playground?.visual?.caption || (isRecap ? 'Well Done!' : 'Concept Visualization')}
      </h3>
      
      <p className="text-maru-grey max-w-sm mb-8">
        {isRecap 
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-maru-cloud">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-maru-navy mb-2">Lesson not found</h1>
          <p className="text-maru-grey mb-4">The requested lesson doesn't exist.</p>
          <button 
